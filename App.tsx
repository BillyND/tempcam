...
const App: React.FC = () => {
  ...
  if (!isDbReady || !settings) {
    return <div className="h-screen-dvh w-full bg-black" />;
  }

  // Router logic without bottom nav
  return (
    <div className="h-screen-dvh w-full bg-black text-white overflow-hidden">
      <Toaster position="top-center" />

      {view === "camera" && (
        <CameraScreen settings={settings} onNavigate={(v) => setView(v)} />
      )}

      {view === "gallery" && <GalleryScreen onBack={() => setView("camera")} />}
